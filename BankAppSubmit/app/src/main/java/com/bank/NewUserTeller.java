...
public class NewUserTeller extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(R.layout.activity_new_user_teller);
    ...
    button.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View view) {
        if (useridauthen1(context)) {
          Intent i = new Intent(NewUserTeller.this, TELLERmain.class);
  ...
  public boolean useridauthen1(Context context) {
    ...
    if (useridAuthen && passwordAuthen && ageAuthen && addressAuthen) {
      passed = runNewCustomer(context);
    }
    return (passed);
  }

  private boolean runNewCustomer(Context context) {
    int success = -1;
    EnumMapRoles roleMap = new EnumMapRoles(context);
    // Insert info into database
    try {
      success = DatabaseInsertHelper.insertNewUser(name, age, add, roleMap.get(Roles.CUSTOMER), pass, context);
    } catch (InvalidNameException e) {
      e.printStackTrace();
    }
    // If insert failed, print statement
    if (success == -1) {
      Toast.makeText(getApplicationContext(), "Customer was not added", Toast.LENGTH_LONG).show();
      return false;
    } else {
      Toast.makeText(getApplicationContext(), "Customer id is: " + String.valueOf(success),
