...
public class MakeWithdrawl extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(R.layout.activity_make_withdrawl);

    context = this.getApplication();
    terminal = (TellerTerminal) getIntent().getSerializableExtra("terminal");

    Button button = (Button) findViewById(R.id.amountTellerSubmit);
    button.setOnClickListener(new View.OnClickListener() {

      public void onClick(View view) {
        if (useridauthen1(context)) {
          Intent i = new Intent(MakeWithdrawl.this, TellerCustomerLogin.class);
  ...
  private boolean useridauthen1(Context context) {
    ...
    EditText etAmount = (EditText) findViewById(R.id.amountTeller);
    ...
    if (passwordauthen && useridauthen) {
      passed = runWithdraw(true, context);
    }
    return (passed);
  }
      ...
      for (int i = 0; i < listAcc.size(); i++) {
        if (accountName.equals(listAcc.get(i).getName())) {
          ...
          if (fromTeller && (DatabaseSelectHelper.getAccountDetails(accId, context).getType()
            ...
            Toast.makeText(getApplicationContext(), "Cannot make a withdrawal to a RestrictedSavingsAccount from a"
                    + " teller terminal.", Toast.LENGTH_LONG).show();
          } else {
            if (!fromTeller) {
