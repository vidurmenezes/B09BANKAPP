...
public class WithdrawalCommunicator {

  private static BufferedReader input = new BufferedReader(new InputStreamReader(System.in));

  public static BigDecimal makeWithdrawal(boolean fromTeller, Context context) throws IOException {
    ...
    try {
      ...
      amount = new BigDecimal(input.readLine());
      ...
      for (int i = 0; i < listAcc.size(); i++) {
        if (accInput.equals(listAcc.get(i).getName())) {
          ...
          if (fromTeller && (DatabaseSelectHelper.getAccountDetails(accId, context).getType()
            ...
            if (!fromTeller) {
              success = AtmCommunicator.makeWithdrawal(amount, accId, context);
            } else {
              success = TellerCommunicator.makeWithdrawal(amount, accId, context);
            }
          }
          break;
        }
      ...
      if (DatabaseSelectHelper.getAccountType(accId, context) == accountsMap.get(AccountTypes.SAVING)) {
        ...
        if (account.getBalance().compareTo(new BigDecimal("1000")) == -1) {
          DatabaseUpdateHelper.updateAccountType(accountsMap.get(AccountTypes.CHEQUING), accId,
              context);
          System.out.println(
              "Since this savings account dropped below $1000, it is now a" + " chequing account");
